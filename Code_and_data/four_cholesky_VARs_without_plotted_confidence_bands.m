%% Four Cholesky VAR specifications with seasonal dummies
%
% Model suffixes:
...
%
% The solid black line is the OLS Cholesky impulse response.
% Confidence bands are not plotted. The four-model figure structure, panel
% titles, axis limits, axis ticks and scaling are otherwise unchanged.
%
% All models return and plot only the output and price-level responses.
% Policy variables and commodity prices remain in the estimated VARs but
% their responses are not plotted. Eleven monthly seasonal dummies are
% included as deterministic regressors in every equation of every VAR.
...
% The existing IRF helper still expects its draw and stability options.
% These settings are retained for compatibility, but the resulting confidence
% bands and draw statistics are not stored or plotted by this script.
...
function [figure_handle, layout_handle, axes_handles] = ...
        plot_tagged_irfs( ...
        irfs_for_plot, horizons, panel_indices, panel_titles, ...
        y_axis_labels, y_axis_limits, y_axis_ticks, ...
        figure_position, figure_title)
%PLOT_TAGGED_IRFS Plot OLS IRFs using the original tiled-panel structure.
