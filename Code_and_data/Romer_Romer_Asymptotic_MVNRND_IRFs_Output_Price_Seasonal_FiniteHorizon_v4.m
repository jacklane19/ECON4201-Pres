%% Four Cholesky VAR specifications with seasonal dummies, finite-horizon MVNRND draws and confidence bands
%
% Model suffixes:
...
% No strict stationarity filter is imposed on the parameter draws. Loose
% numerical safeguards remove only clearly explosive roots and extreme
% finite-horizon IRFs.
...
%% Requirements
%
% Place var_irf_asymptotic_mvnrnd_seasonal_finite_horizon_v4.m in the same folder as this script.
...
% Loose numerical safeguards for a persistent levels VAR.
%
% This is not a stationarity restriction. Draws with roots at or slightly
% above one remain eligible. Only clearly explosive roots above 1.05 are
% rejected to prevent a few numerical explosions from dominating the
% finite-horizon standard deviations.
maximum_allowed_root = 1.05;

% Maximum absolute normalised IRF, measured in the original model units,
% allowed anywhere from horizon 0 through 48. The value 100 is deliberately
% loose and is intended only to remove numerical explosions.
maximum_allowed_absolute_irf = 100;

% Continue drawing until the requested number of valid finite-horizon draws
% is retained. Increase this limit if many candidates hit the safeguards.
maximum_draw_attempts = 50000;
draw_batch_size = 500;
...
asymptotic_options = struct( ...
    ...
    'maximum_allowed_root', maximum_allowed_root, ...
...
VAR_newm = var_irf_asymptotic_mvnrnd_seasonal_finite_horizon_v4( ...
...
fprintf(['_newm draw attempts: %d; rejected by loose root ', ...
    'safeguard: %d; rejected extreme IRFs: %d; rejected invalid: %d; ', ...
    'acceptance rate: %.2f%%\n'], ...
    VAR_newm.attempted_number_of_draws, ...
    VAR_newm.rejected_root_safeguard_draws, ...
...
VAR_newm_com = var_irf_asymptotic_mvnrnd_seasonal_finite_horizon_v4( ...
...
fprintf(['_newm_com draw attempts: %d; rejected by loose root ', ...
    'safeguard: %d; rejected extreme IRFs: %d; rejected invalid: %d; ', ...
    'acceptance rate: %.2f%%\n'], ...
    VAR_newm_com.attempted_number_of_draws, ...
    VAR_newm_com.rejected_root_safeguard_draws, ...
...
VAR_ff = var_irf_asymptotic_mvnrnd_seasonal_finite_horizon_v4( ...
...
fprintf(['_ff draw attempts: %d; rejected by loose root ', ...
    'safeguard: %d; rejected extreme IRFs: %d; rejected invalid: %d; ', ...
    'acceptance rate: %.2f%%\n'], ...
    VAR_ff.attempted_number_of_draws, ...
    VAR_ff.rejected_root_safeguard_draws, ...
...
VAR_ff_com = var_irf_asymptotic_mvnrnd_seasonal_finite_horizon_v4( ...
...
fprintf(['_ff_com draw attempts: %d; rejected by loose root ', ...
    'safeguard: %d; rejected extreme IRFs: %d; rejected invalid: %d; ', ...
    'acceptance rate: %.2f%%\n'], ...
    VAR_ff_com.attempted_number_of_draws, ...
    VAR_ff_com.rejected_root_safeguard_draws, ...
