%% Four Cholesky VAR specifications with seasonal dummies, stable MVNRND draws and confidence bands
%
% Model suffixes:
...
% as deterministic regressors in every equation of every VAR. Parameter
% draws are retained only when the maximum companion-matrix root is below
% the editable stability threshold.
...
%% Requirements
%
% Place var_irf_asymptotic_mvnrnd_seasonal_stability_v3.m in the same folder as this script.
...
% Stability filter for simulated VAR coefficient draws. A draw is retained
% only when its largest absolute companion-matrix eigenvalue is below this
% value. Set to 1 for the conventional strict stability condition. The
% slightly tighter 0.999 threshold also removes near-explosive draws.
stability_threshold = 0.999;

% Continue drawing until the requested number of stable draws is retained.
% Increase this limit if a persistent, high-lag VAR rejects many draws.
...
VAR_newm = var_irf_asymptotic_mvnrnd_seasonal_stability_v3( ...
...
fprintf(['_newm stable-draw attempts: %d; rejected unstable: %d; ', ...
    'rejected invalid: %d; acceptance rate: %.2f%%\n'], ...
...
VAR_newm_com = var_irf_asymptotic_mvnrnd_seasonal_stability_v3( ...
...
fprintf(['_newm_com stable-draw attempts: %d; rejected unstable: %d; ', ...
    'rejected invalid: %d; acceptance rate: %.2f%%\n'], ...
...
VAR_ff = var_irf_asymptotic_mvnrnd_seasonal_stability_v3( ...
...
fprintf(['_ff stable-draw attempts: %d; rejected unstable: %d; ', ...
    'rejected invalid: %d; acceptance rate: %.2f%%\n'], ...
...
VAR_ff_com = var_irf_asymptotic_mvnrnd_seasonal_stability_v3( ...
...
fprintf(['_ff_com stable-draw attempts: %d; rejected unstable: %d; ', ...
    'rejected invalid: %d; acceptance rate: %.2f%%\n'], ...
