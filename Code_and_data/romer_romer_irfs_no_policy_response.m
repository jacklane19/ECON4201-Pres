%% Four Cholesky VAR specifications with asymptotic MVNRND confidence bands
%
% Model suffixes:
...
% intervals for the requested responses and horizons. Commodity prices
% remain inside the two four-variable VARs, but their IRFs are not returned,
% stored or plotted.
...
%% Requirements
%
% Place var_irf_asymptotic_mvnrnd.m in the same folder as this script.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Assumption tests: VAR lag-length selection
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
maximum_number_of_regressors = 1 + number_of_variables * maximum_lag;
...
for lag = 1:maximum_lag
    ...
    [estimated_models{lag}, ~, log_likelihood(lag)] = estimate( ...
        model, Y_estimation, 'Y0', Y_presample, 'Display', 'off');
...
%% Nelson-Plosser stationarity tests
%
% Level test:
% H0: The variable has a unit root.
% H1: The variable is stationary around a deterministic linear trend.
%
% First-difference test:
% H0: The first difference has a unit root.
% H1: The first difference is stationary around a constant mean.
...
% Maximum number of lagged differences considered.
% This does not need to equal the number of VAR lags.
max_lag = 24;

% Significance level used when testing down the augmentation lags.
lag_significance = 0.10;

%% Preallocate results

number_of_variables = size(Y, 2);

observations      = zeros(number_of_variables, 1);

level_lag         = zeros(number_of_variables, 1);
level_reject      = false(number_of_variables, 1);
level_p_value     = zeros(number_of_variables, 1);
level_statistic   = zeros(number_of_variables, 1);
level_critical    = zeros(number_of_variables, 1);

difference_lag       = zeros(number_of_variables, 1);
difference_reject    = false(number_of_variables, 1);
difference_p_value   = zeros(number_of_variables, 1);
difference_statistic = zeros(number_of_variables, 1);
difference_critical  = zeros(number_of_variables, 1);

classification = strings(number_of_variables, 1);

%% Conduct tests

for variable_index = 1:number_of_variables

    % Extract one series and remove missing or infinite observations
    y = Y(:, variable_index);
    y = y(isfinite(y));

    observations(variable_index) = length(y);

    if length(y) < 20
        error( ...
            'Variable %s has too few usable observations.', ...
            variable_names(variable_index));
    end

    if std(y) == 0
        error( ...
            'Variable %s is constant and cannot be tested.', ...
            variable_names(variable_index));
    end

    %% 1. Test the variable in levels

    % Trend-stationary alternative:
    % y_t = c + delta*t + rho*y_(t-1)
    %       + sum gamma_j*Delta y_(t-j) + error_t

    level_lag(variable_index) = select_adf_lag( ...
        y, ...
        'TS', ...
        max_lag, ...
        lag_significance);

    [level_reject(variable_index), ...
     level_p_value(variable_index), ...
     level_statistic(variable_index), ...
     level_critical(variable_index)] = adftest( ...
        y, ...
        'Model', 'TS', ...
        'Lags', level_lag(variable_index), ...
        'Test', 't1', ...
        'Alpha', alpha);

    %% 2. Test the first difference

    delta_y = diff(y);

    difference_lag(variable_index) = select_adf_lag( ...
        delta_y, ...
        'ARD', ...
        max_lag, ...
        lag_significance);

    [difference_reject(variable_index), ...
     difference_p_value(variable_index), ...
     difference_statistic(variable_index), ...
     difference_critical(variable_index)] = adftest( ...
        delta_y, ...
        'Model', 'ARD', ...
        'Lags', difference_lag(variable_index), ...
        'Test', 't1', ...
        'Alpha', alpha);

    %% 3. Classify the series

    if level_reject(variable_index)

        classification(variable_index) = ...
            "Trend-stationary in levels";

    elseif difference_reject(variable_index)

        classification(variable_index) = ...
            "Difference-stationary: approximately I(1)";

    else

        classification(variable_index) = ...
            "Unit root remains after first difference / inconclusive";
...
nelson_plosser_results = table( ...
    ...
    'VariableNames', { ...
        ...
        'LevelCriticalValue', ...
        'DifferenceLag', ...
        'DifferenceRejectUnitRoot', ...
        'DifferencePValue', ...
        'DifferenceStatistic', ...
        'DifferenceCriticalValue', ...
        'Classification' ...
        });

disp(' ');
disp('Nelson-Plosser unit-root test results');
disp(nelson_plosser_results);


%% Local function: select ADF augmentation lag
%
% Starts with max_lag and removes the final lag if its coefficient is not
% statistically significant. This continues until the final lag is
% significant or the lag length reaches zero.

function selected_lag = select_adf_lag( ...
    y, model_type, max_lag, lag_significance)

    y = y(isfinite(y));
    number_of_observations = length(y);

    % Ensure there are sufficient observations for the regression.
    maximum_feasible_lag = max( ...
        0, ...
        floor((number_of_observations - 5) / 2));

    selected_lag = min(max_lag, maximum_feasible_lag);

    while selected_lag > 0

        [~, ~, ~, ~, regression_results] = adftest( ...
            y, ...
            'Model', model_type, ...
            'Lags', selected_lag, ...
            'Test', 't1');

        % MATLAB names the lagged difference coefficients b1, b2, ...
        final_lag_name = sprintf('b%d', selected_lag);

        final_lag_index = find( ...
            strcmp(regression_results.names, final_lag_name), ...
            1);

        if isempty(final_lag_index)
            error( ...
                'Could not locate coefficient %s.', ...
                final_lag_name);
        end

        final_lag_p_value = ...
            regression_results.tStats.pVal(final_lag_index);

        % Keep this lag if the final augmentation term is significant
        if final_lag_p_value <= lag_significance
            return
        end

        % Otherwise remove the final lag and estimate again
