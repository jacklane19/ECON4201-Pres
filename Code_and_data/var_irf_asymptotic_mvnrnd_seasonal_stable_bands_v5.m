function results = var_irf_asymptotic_mvnrnd_seasonal_stable_bands_v5( ...
        Y, lags, maximum_horizon, policy_index, number_of_draws, options)
%VAR_IRF_ASYMPTOTIC_MVNRND_SEASONAL_STABLE_BANDS_V5 Frequentist bands for a Cholesky VAR.
%
...
%   stability_threshold           Default: 1.0
%   maximum_allowed_absolute_irf  Default: 100
%   maximum_draw_attempts         Default: max(20000,200*number_of_draws)
...
% ------------------------------
% The original OLS Cholesky IRF is always retained as the point estimate,
% even when the estimated OLS VAR has a companion root at or above one.
% However, pointwise standard errors and confidence intervals are computed
% only from dynamically stable parameter draws. A candidate draw is kept
% only when its largest absolute companion root is strictly below
% options.stability_threshold (normally 1.0).
%
% A separate loose finite-horizon safeguard rejects a stable draw if its
% maximum absolute normalised IRF exceeds maximum_allowed_absolute_irf.
% The routine continues drawing until the requested number of stable, valid
...
% and NORMINV.
    ...
    if ~isfield(options, 'stability_threshold') || ...
        ...
        options.stability_threshold = 1.0;
    end
    ...
    if ~isfield(options, 'maximum_draw_attempts') || ...
        ...
        options.maximum_draw_attempts = max(20000, 200 * number_of_draws);
    end
    ...
    if ~isfinite(maximum_companion_root_ols)
        warning(['The OLS companion root could not be computed reliably. ', ...
            'The original OLS IRF is still retained as the point estimate, ', ...
            'while confidence bands use only valid stable draws.']);
    elseif ~ols_is_dynamically_stable
        warning(['The estimated OLS VAR is dynamically unstable: the ', ...
            'largest absolute companion root is %.6f. The original OLS ', ...
            'IRF is nevertheless retained as the point estimate. The ', ...
            'pointwise standard errors and confidence intervals are ', ...
            'computed only from parameter draws with roots strictly below ', ...
            '%.6f.'], ...
    ...
    %
    % The OLS IRF above remains unchanged. The uncertainty distribution is
    % conditional on dynamic stability: only draws with maximum companion
    % root strictly below options.stability_threshold are retained.
    ...
    while accepted_draws < number_of_draws && ...
        ...
        for draw_in_batch = 1:current_batch_size
            ...
            if maximum_companion_root_draw >= options.stability_threshold
    ...
    if accepted_draws < number_of_draws
        error(['Only %d stable, valid draws were accepted after %d ', ...
            'attempts. %d draws were dynamically unstable ', ...
            '(maximum root >= %.6f), %d exceeded the absolute-IRF ', ...
            'safeguard (maximum absolute IRF > %.6f), and %d were ', ...
            'invalid. Increase maximum_draw_attempts if the stable-draw ', ...
            'acceptance rate is low.'], ...
            accepted_draws, attempted_draws, ...
            rejected_unstable_draws, ...
    ...
    %% Pointwise standard errors and intervals from stable draws only
    ...
    results.irf_pointwise_std_stable_draws = irf_pointwise_std;
    results.confidence_bands_condition_on_stable_draws = true;
    ...
    results.rejected_unstable_draws = ...
        rejected_unstable_draws;
